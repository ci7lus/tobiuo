...
router.get("/", async (ctx) => {
  if (ctx.session.token) {
    const subscriptionsReq = await Axios.get(
      new URL(urljoin(endpoint, "/api/v1/webpush/subscriptions")).href,
      {
        headers: {
...
router.get("/user/login", async (ctx) => {
  ...
  if (ctx.query.code && ctx.query.state) {
    ...
    const account = await Axios.get(
      new URL(urljoin(endpoint, "/api/v1/account")).href,
      {
        headers: {
          Authorization: `Bearer ${ctx.session.token}`,
        },
...
router.post("/", body(), async (ctx) => {
  ...
  const uri: string = await (async () => {
    ...
    let exc
    if (
      (exc = /https\:\/\/discordapp.com\/api\/webhooks\/[0-9]+\/[A-Za-z0-9-_]+/.exec(
        request
      ))
    ) {
      ...
      if (
        subscriptionsReq.data.filter(
          (subscription: any) => subscription.description == urig
        ).length
      )
        return ctx.throw(400, "既に登録済みっぽい")
      return urig
    } else if (
      (exc = /https\:\/\/hooks.slack.com\/services\/[A-Z0-9]+\/[A-Z0-9]+\/[A-Za-z0-9]+/.exec(
        request
      ))
    ) {
      const urig = exc[0]
...
router.get("/:id", async (ctx) => {
  ...
  const subscriptionFil = subscriptionsReq.data.filter(
    (subscription: any) => subscription.id == id
  )
  ...
  if (ctx.query.delete == "1") {
    await Axios.delete(
      new URL(urljoin(endpoint, `/api/v1/webpush/subscriptions/${id}`)).href,
      {
        headers: {
          Authorization: `Bearer ${ctx.session.token}`,
        },
...
router.post("/push/:token", async (ctx) => {
  if (ctx.request.headers["content-encoding"] != "aes128gcm")
    ctx.throw(400, "content-encoding != aes128gcm")
  const body = await new Promise<Buffer>((resolve) =>
    ctx.req.pipe(concatStream(resolve))
  )
